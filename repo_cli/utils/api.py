...
class RepoApi:
    ...
    def get_channel_artifacts(self, channel, offset=None, limit=None):
        ...
        resp = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
        return self._manage_response(resp, "getting artifacts").get("items", [])

    def get_channel_artifacts_files(
        self, channel, family, package=None, version=None, filename=None
    ):
        file_family_parsers = {
        ...
        for package in packages:
            url = join(
                self._get_channel_url(channel), "artifacts", family, package, "files"
            )
            resp = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
